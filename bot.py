...
def send_welcome(message):
    ...
    listPhoto = []
    myReviews = []
    bot.reply_to(message,f'{message.from_user.first_name}, Вы вышли из системы. Для повторного входа воспользуйтесь /start')
...
def callback(call):
    ...
    if domen and token and contLang:
        if values == 'products':
            ...
            btn_attr_count = types.InlineKeyboardButton('\U00002699 Вывод атрибутов', callback_data='attr_count')
            btn_back = types.InlineKeyboardButton('\U0001F519 Назад', callback_data='back')
            markup.add(btn_lim, btn_dinfo, btn_rev, btn_about, btn_attr_count, btn_back)
...
'''
        ...
        elif values == 'attr_count':
            markup = types.InlineKeyboardMarkup(row_width=1)
            btn_back = types.InlineKeyboardButton('\U0001F519 Назад', callback_data='back')
            markup.add(btn_back)
            bot.send_message(call.message.chat.id, 'Вы можете настроить опцию для максимального количества выводимых атрибутов при поиске товара.\nПо умолчанию - 15.\nРекомендуется не более 50, иначе можно сломать бота, так как у Telegram ограничение по выводимым символам.\nДля изменения, введите значение:', reply_markup=markup)
            bot.register_next_step_handler(call.message, change_attr_count)
        ...
        if domen and token and contLang:
            ...
            try:
                ...
                try:
                    ...
                    params = {
                            'filters': '{"attribute_type": '+ pn_type +', "id": '+ pn_id +', "exp": "in", "name": "' + pn_name + '", "lang": "'+ contLang + '", "pickle__strong": true, "val": ["'+ message.text +'"], "ref_list_id": null,"type": "'+ pn_id +'"}'}
                    ...
                    sku = prodRequest['results'][0]['meta']['id']
                    pn = prodRequest['results'][0]['meta']['name']
                    result_attrs = ''
                    attrs_count = 0
                    if len(attributes.values()) != 0:
                        for attr in attributes.values():
                    ...
                    btn_attr_count = types.InlineKeyboardButton('\U00002699 Уменьшить значение', callback_data='attr_count')
                    markup.add(btn_attr_count)
                    bot.send_message(message.chat.id, 'Товар не найден.\nВозможно проблема в количестве выводимых атрибутов, попробуйте уменьшить значение', reply_markup=markup)
        else:
...
def get_review(message): # Записывает отзыв в бд
    ...
    try:
        ...
        conn = sqlite3.connect('users_data.sql')
        cur = conn.cursor()
        cur.execute("INSERT INTO reviews (date, username, review, tgid) VALUES ('%s', '%s', '%s', '%s')" % (now, message.from_user.first_name, message.text, message.from_user.id))
        conn.commit()
...
def change_attr_count(message):
    global attrCount
    try:
        attrCount = int(message.text)
        markup = types.InlineKeyboardMarkup(row_width=1)
        btn_back = types.InlineKeyboardButton('\U0001F519 Назад', callback_data='back')
        markup.add(btn_back)
        bot.send_message(message.chat.id, f'Отлично, вы изменили количество выводимых атрибутов при поиске товара до {attrCount}', reply_markup=markup)
    except:
        markup = types.InlineKeyboardMarkup(row_width=1)
        btn_back = types.InlineKeyboardButton('\U0001F519 Назад', callback_data='back')
        markup.add(btn_back)
        bot.send_message(message.chat.id, 'Введено некорректное значение, должно быть целое число!', reply_markup=markup)
